...
├── signing_key.go              # signing-keys resource, KeyID(kid) derivation, SigningKeySet
├── checkout_key_set.go         # VerifyWithKeySet for both file types — survives key rotation
...
- **`CanonicalFingerprint` must never grow hardware-identifier reading.** It is a pure function
  over caller-supplied labelled components (`fingerprint.go`). What identifies a machine is a
  product decision — a cloned VM template shares its serials, a container has none, a replaced
  motherboard changes them — and eight SDKs each guessing would disagree silently, which is
  double-billing rather than a visible failure. For the same reason values are **not**
  Unicode-normalised: NFC is a rule the eight ports cannot implement identically. Invalid input
  is an error, never a silent repair, because stripping a control character or deduplicating a
  repeated label maps two different inputs onto one seat — the exact bug the function exists to
  prevent. `testdata/fingerprint-vectors.json` is the cross-SDK vector set and was **not**
  generated by any SDK; do not regenerate it from this implementation.
...
- **`kid` hashes the base64 STRING, not the decoded key bytes.** `key_id`
  (`shared/crypto/license_file.rs:70`) digests the stored base64 `&str` and keeps the first 8
  bytes as lowercase hex. Decoding first is the natural assumption and it is wrong — it yields
  an id matching nothing the server emits, and every rotation-aware verification then reports
  authentic files as signed by an unknown key. `testdata/signing-key-ids.json` (generated by an
  independent SHA-256 implementation, **not** by any SDK) pins both the correct value and the
  wrong one for the same key; `signing_key_test.go` asserts against both, because the positive
  vector alone cannot tell a correct port from a lucky one.
- **`GET /signing-keys` 403s under licence-key auth.** It authorizes on `account.read`, which is
  not in the `LicenseToken` role's permission set (`authz/mod.rs`) — same shape as
  `GET /policies/{id}`, but unlike that one there is **no** equivalent route reachable through a
  permission the role does hold. An embedded client must pin public keys in its binary
  (`NewSigningKeySetFromPublicKeys`) or proxy the call through its own backend.
- **A `.machine` file's `kid` names the account's Ed25519 key whatever scheme signed it.**
  `check_out_machine.rs:127` derives the claim from `account.ed25519_public_key` unconditionally,
  while `:86-99` picks the *signing* key by scheme. So an RSA/ECDSA machine file carries a real
  `kid` pointing at a published key that cannot possibly verify it — looking that key up would
  report an authentic file as forged. `(*MachineFile).VerifyWithKeySet` therefore refuses any
  scheme but `Ed25519Sign` up front with `ErrNoUsableSigningKey`. Those keys are never published
  and never rotated (`rotate_ed25519` is the only rotation), so the rotation defect cannot arise
  for them anyway. ⚠️ Note the fixture manifest in `testdata/server-machine-file-fixtures/`
  disagrees: its generator derived each `kid` from that file's own signing key, so the RSA and
  ECDSA entries' `kid`s are not what a production checkout would carry. They still pin the hash
  rule, which is all `signing_key_test.go` asserts from them.
